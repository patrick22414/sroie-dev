...
from .models import GRID_H, GRID_W, RESO_H, RESO_W

DATA_PATH = "../sroie-data/"

def get_train_data(batch_size, device):
    filenames = [os.path.splitext(f)[0] for f in glob.glob(DATA_PATH + "data_train/*.jpg")]
...
def get_eval_data(batch_size, device):
    jpg_files = random.sample(glob.glob(DATA_PATH + "data_valid/*.jpg"), batch_size)
